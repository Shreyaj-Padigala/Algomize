...
/**
 * Condition One Agent — evaluates the user's first trading condition
 * on a scale of 1-10 for both long and short positions.
 *
 * Uses a combination of technical indicators (RSI, EMA, market structure)
 * and AI interpretation of the user's condition description to score.
 */
class ConditionOneAgent {
  constructor() {
    this.name = 'condition_1';
  ...
  async analyze(candles15m, candles1h, conditionDescription) {
    ...
    // Gather market data for AI context
    const marketData = this._gatherMarketData(candles15m, candles1h);

    // Ask Nova to evaluate the condition
  ...
  _gatherMarketData(candles15m, candles1h) {
    ...
    // Price change over recent periods
    ...
    // ASCII chart for visual context
...
Respond ONLY with valid JSON: {"longScore": X, "shortScore": Y, "summary": "..."}`;
    ...
    try {
      ...
      // Fallback: use basic technical analysis
  ...
  _fallbackEvaluation(marketData) {
    ...
    return { longScore, shortScore, summary: `RSI: ${marketData.rsi} | Trend: ${marketData.trend} (AI unavailable, using fallback)` };
  }
}

module.exports = ConditionOneAgent;
